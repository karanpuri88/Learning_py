# Coded by Karan Puri (karpuri)

# Issue : cEdge admin Tech has lots of .bin files & TAC engineers may not aware which bin file they need to decode for further analyse 

# Solution : Decode full admin tech in one go using smart decoder & based on user input snip (grep) the logs for TAC engineer for further analysis 

# Enter Case number , Auto download , create folder , upload to smart decoder , move decoded file , search string 

# Advance Level : Provide GUI 
...
# Upload admin tech to smart decoder 

# Move decoded admin tech to previous 

# Find a string in all Folders 

# Grep all logs wrt to time stamp 1 Min Above & 1 Min below
